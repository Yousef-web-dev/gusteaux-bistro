    ...
    <div>
      ...
      <section className="relative overflow-hidden bg-stone-900">
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-24 pb-28 sm:pt-32 sm:pb-36">
          ...
          >
            ...
            <p className="mt-6 text-lg text-cream-100/80 leading-relaxed max-w-xl">
              Gusteau&apos;s welcomes you to a table where every dish is
              cooked with total attention — from a humble ratatouille to a
              masterwork of French pastry. Taste the story behind every
              plate.
            </p>
            <div className="mt-9 flex flex-wrap gap-4">
              ...
              <Link
                href="/contact"
                className="inline-flex items-center gap-2 rounded-md border border-navy-100/40 hover:bg-navy-100/10 text-cream-50 px-6 py-3.5 font-medium transition-colors"
              >
                Reserve a Table
              </Link>
            </div>
        ...
        <div className="pointer-events-none absolute bottom-0 right-10 hidden lg:flex gap-2 opacity-60">
          <span className="w-1 h-16 rounded-full bg-gradient-to-t from-transparent to-cream-100/40 animate-steam" style={{ animationDelay: "0s" }} />
          <span className="w-1 h-20 rounded-full bg-gradient-to-t from-transparent to-cream-100/40 animate-steam" style={{ animationDelay: "0.6s" }} />
          <span className="w-1 h-12 rounded-full bg-gradient-to-t from-transparent to-cream-100/40 animate-steam" style={{ animationDelay: "1.2s" }} />
        </div>
      </section>
      ...
      <section className="bg-cream-100 dark:bg-night-800 border-y border-stone-300/30 dark:border-navy-400/20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 grid grid-cols-1 sm:grid-cols-3 gap-8 text-center">
          {[
            { icon: ChefHat, title: "Since 1962", text: "A recipe book passed down through generations." },
            { icon: UtensilsCrossed, title: "Fresh, Daily", text: "Sourced each dawn from the Rue Cler market." },
            { icon: Sparkles, title: "Cooked with Heart", text: "A great artist can come from anywhere." },
          ].map((item, i) => (
            <motion.div
              key={item.title}
